...
use crate::keyring::*;
use benson_primitives::types::{AccountId, Block, Signature};
use benson_runtime::{
	constants::{asset::SPENDING_ASSET_ID, currency::DOLLARS},
...
use sp_consensus::{BlockImport, BlockImportParams, BlockOrigin, ForkChoiceStrategy, ImportResult, ImportedAux};
...
impl Clone for BenchDb {
	fn clone(&self) -> Self {
		...
		let seed_db_files = std::fs::read_dir(seed_dir)
			...
			.map(|f_result| f_result.expect("failed to read file in seed db").path().clone())
			.collect();
...
impl BenchDb {
	...
	pub fn generate_block(&mut self, transactions: usize) -> Block {
		let (client, _backend) = Self::bench_client(self.directory_guard.path(), Profile::Wasm, &self.keyring);
		...
		for extrinsic in client
			.runtime_api()
			.inherent_extrinsics_with_context(&BlockId::number(0), ExecutionContext::BlockConstruction, inherent_data)
		...
		for _ in 0..transactions {
			...
			let receiver = get_account_id_from_seed::<sr25519::Public>(&format!("random-user//{}", iteration));

			let signed = self.keyring.sign(
				CheckedExtrinsic {
					signed: Some((sender, signed_extra(0, 1 * DOLLARS, None, None))),
					function: Call::GenericAsset(GenericAssetCall::transfer(SPENDING_ASSET_ID, receiver, 1 * DOLLARS)),
				},
				version,
				genesis_hash,
			);
...
impl BenchKeyring {
	...
	pub fn generate_genesis(&self) -> benson_runtime::GenesisConfig {
		crate::genesis::config_endowed(false, Some(benson_runtime::WASM_BINARY), self.collect_account_ids())
	}
}
