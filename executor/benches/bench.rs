...
use benson_runtime::constants::{asset::SPENDING_ASSET_ID, currency::*};
use benson_runtime::{
	Block, BuildStorage, Call, CheckedExtrinsic, GenericAssetCall, GenesisConfig, Header, UncheckedExtrinsic,
};
...
fn test_blocks(genesis_config: &GenesisConfig, executor: &NativeExecutor<Executor>) -> Vec<(Vec<u8>, Hash)> {
	...
	block1_extrinsics.extend((0..20).map(|i| CheckedExtrinsic {
		signed: Some((alice(), signed_extra(i, 0, None, None))),
		function: Call::GenericAsset(GenericAssetCall::transfer(SPENDING_ASSET_ID, bob().into(), 1 * DOLLARS)),
	}));
...
fn bench_execute_block(c: &mut Criterion) {
	c.bench_function_over_inputs(
		"execute blocks",
		|b, strategy| {
			let genesis_config = benson_testing::genesis::config(false, Some(COMPACT_CODE));
