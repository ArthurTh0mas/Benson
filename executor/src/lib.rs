...
mod tests {
	use super::Executor;
	use benson_primitives::{Balance, BlockNumber, Hash, Index, Timestamp};
	use benson_runtime::{
	...
	/// runtime.
	const COMPACT_CODE: &[u8] = benson_runtime::WASM_BINARY;
	...
	const BLOATY_CODE: &[u8] = benson_runtime::WASM_BINARY_BLOATY;

	const GENESIS_HASH: [u8; 32] = [69u8; 32];

	const VERSION: u32 = benson_runtime::VERSION.spec_version;
	...
	fn full_native_block_import_works() {
		...
		t.execute_with(|| {
			...
			assert_eq!(Balances::total_balance(&bob()), 179 * DOLLARS - transfer_fee(&xt(), fm),);
			let events = vec![
				EventRecord {
					phase: Phase::ApplyExtrinsic(0),
					event: Event::system(system::Event::ExtrinsicSuccess),
...
"#;
	...
	fn should_import_block_with_test_client() {
		use benson_testing::client::{consensus::BlockOrigin, ClientExt, TestClientBuilder, TestClientBuilderExt};

		let client = TestClientBuilder::new().build();
		let block1 = changes_trie_block();
		let block_data = block1.0;
		let block = benson_primitives::Block::decode(&mut &block_data[..]).unwrap();
	...
	fn transaction_fee_is_correct_ultimate() {
		// This uses the exact values of benson-node.
		//
	...
	mod benches {
		...
		fn wasm_execute_block(b: &mut Bencher) {
			...
			b.iter(|| {
				let mut t = new_test_ext(COMPACT_CODE, false);
				WasmExecutor::new()
					.call(&mut t, "Core_execute_block", &block1.0)
					.unwrap();
				WasmExecutor::new()
					.call(&mut t, "Core_execute_block", &block2.0)
					.unwrap();
			});
		}
	}
