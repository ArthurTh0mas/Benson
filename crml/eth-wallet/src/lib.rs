...
//! Provides shims for Ethereum wallets (e.g. metamask) to interact with Benson
...
decl_event!(
	...
	{
		/// A call just executed. (Ethereum Address, Benson Address, Result)
...
decl_storage! {
	trait Store for Module<T: Config> as EthWallet {
		/// Mapping from Ethereum address to a Benson only transaction nonce
		/// It may be higher than the Benson system nonce for the same address
	...
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		fn call(
			...
			if let Some(public_key) = ecrecover(&signature, message, &eth_address) {
				let account = T::Signer::from(public_key).into_account(); // Benson address
...
impl<T: Config> Module<T> {
	...
	/// Return the known Benson nonce for a given Ethereum address
...
mod tests {
	...
	fn simple_remark() {
		new_test_ext().execute_with(|| {
			...
			let benson_address = MultiSigner::from(pair.public()).into_account();

			let call: Call = frame_system::Call::<Runtime>::remark(b"hello world".to_vec()).into();
			let system_nonce = <frame_system::Module<Runtime>>::account_nonce(&benson_address);
			...
			assert_eq!(
				<frame_system::Module<Runtime>>::account_nonce(&benson_address),
				system_nonce + 1,
			);
