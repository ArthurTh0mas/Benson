...
import BensonBridge from '../artifacts/contracts/BensonBridge.sol/BensonBridge.json';
import TestToken from '../artifacts/contracts/TestToken.sol/TestToken.json';

use(solidity);

describe('BensonBridge', () => {
  ...
  beforeEach(async () => {
    ...
    bridge = await deployContract(wallet, BensonBridge, []);
  });
  ...
  it('deposit success', async () => {
    let depositAmount = 7;
    let bensonAddress = "0xacd6118e217e552ba801f7aa8a934ea6a300a5b394e7c3f42cd9d6dd9a457c10";
    await bridge.activateDeposits();
    ...
    await expect(
      bridge.deposit(testToken.address, depositAmount, bensonAddress)
    ).to.emit(bridge, 'Deposit').withArgs(wallet.address, testToken.address, depositAmount, bensonAddress, timestamp);
