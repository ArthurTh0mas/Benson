/* Copyright 2020 Annie Lai Investments Limited
*
...
use benson_primitives::{traits::StakerRewardPayment, types::Exposure};
...
mod tests {
	use super::*;
	use benson_primitives::types::IndividualExposure;
	...
	#[test]
	fn make_reward_payout_development_fund_take() {
		ExtBuilder::default().build().execute_with(|| {
	...
	fn simple_reward_payout_inflation() {
		// the basic reward model on Benson is fees * inflation
		ExtBuilder::default().build().execute_with(|| {
	...
	fn reward_payout_calculations() {
		ExtBuilder::default().build().execute_with(|| {
			...
			Rewards::make_reward_payout(&vec![mock_commission_stake_map.as_tuple()]);
	...
	fn calculate_npos_payouts_empty_stake_map() {
		ExtBuilder::default().build().execute_with(|| {
			let reward = 1_000;
			Rewards::note_transaction_fees(reward);
			Rewards::make_reward_payout(Default::default());
