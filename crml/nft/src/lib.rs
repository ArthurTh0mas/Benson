...
use benson_primitives::types::{AssetId, Balance, BlockNumber};
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin, system = frame_system {
		...
		fn on_initialize(now: T::BlockNumber) -> Weight {
			...
			// https://github.com/ng8eke/benson/issues/444
		...
		fn bid(origin, listing_id: ListingId, amount: Balance) {
			...
			if let Some(Listing::Auction(mut listing)) = Self::listings(listing_id) {
				...
				if let Some(balance_after_bid) = balance.checked_sub(amount) {
					// TODO: review behaviour with 3.0 upgrade: https://github.com/ng8eke/benson/issues/414
					// - `amount` is unused
