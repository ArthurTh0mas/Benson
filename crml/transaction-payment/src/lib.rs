...
// Copyright (C) 2019-2020 Parity Technologies (UK) Ltd. and Annie Lai Investments Ltd.
...
use benson_primitives::{traits::BuyFeeAsset, types::FeeExchange};
	...
	BalanceOf<T>: Send + Sync + FixedPointOperand,
{
	...
	fn withdraw_fee(
		...
		if let Some(exchange) = &self.fee_exchange {
			// Buy the Benson fee currency paying with the user's nominated fee currency
			...
			.execute_with(|| {
				...
				assert_eq!(
					ChargeTransactionPayment::<Runtime>::from(10, Some(fee_exchange))
						...
						.err()
						.unwrap(),
					error_from_code(error_code::MAXIMUM_SELL_REQUIREMENT_NOT_MET)
				);
			})
	}
}
