...
mod doughnut;
mod mock;
use benson_runtime::BensonDoughnut;
...
fn generic_asset_transfer_works_with_doughnut() {
	...
	let mut doughnut = DoughnutV0 {
		...
		signature_version: 0,
		domains: vec![("benson".to_string(), cennznut.encode())],
	};
	...
	let benson_doughnut = BensonDoughnut::new(doughnut);
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let xt = sign(CheckedExtrinsic {
				signed: Some((bob(), signed_extra(0, 0, Some(benson_doughnut), None))),
				function: runtime_call.clone(),
			});
