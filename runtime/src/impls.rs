...
impl<C: Get<Balance>> Convert<Weight, Balance> for LinearWeightToFee<C> {
	fn convert(w: Weight) -> Balance {
		// benson-node a weight of 10_000 (smallest non-zero weight) to be mapped to 10^7 units of
		// fees, hence:
	...
	T: pallet_contracts::Trait + pallet_generic_asset::Trait + crml_cennzx_spot::Trait,
{
	...
	fn fill_gas(transactor: &T::AccountId, gas_limit: Gas) -> Result<GasMeter<T>, DispatchError> {
		// Calculate the cost to fill the meter in the Benson fee currency
		...
		if fee_exchange.is_none() {
			// User will pay for gas in Benson's native fee currency
	...
	///
	/// The process is as follows:
	/// - Default case: refund unused gas tokens to the user (`transactor`) in Benson's native fee currency as the current gas price
	...
	fn empty_unused_gas(transactor: &T::AccountId, gas_meter: GasMeter<T>) {
		...
		if let Some(exchange_op) = storage::unhashed::take::<FeeExchange<T::AssetId, T::Balance>>(&GAS_FEE_EXCHANGE_KEY)
		{
			...
			// Refund remaining gas by minting it as Benson fee currency
...
/// Provides a benson version of doughnut dispatch verification
pub struct BensonDispatchVerifier;

/// Helpers which are used by the DelegatedDispatchVerifier
impl BensonDispatchVerifier {
	...
	fn get_cennznut(doughnut: &<Runtime as frame_system::Trait>::Doughnut) -> Result<CENNZnut, &'static str> {
		let mut domain = doughnut
			.get_domain(<BensonDispatchVerifier as additional_traits::DelegatedDispatchVerifier>::DOMAIN)
			.ok_or("CENNZnut does not grant permission for benson domain")?;
