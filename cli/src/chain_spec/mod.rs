// Copyright 2018-2021 Parity Technologies (UK) Ltd. and Annie Lai Investments Ltd.
...
pub use benson_primitives::types::{AccountId, Balance, Signature};
pub use benson_runtime::GenesisConfig;
...
/// A type to hold keys used in Benson Box in SS58 format.
...
pub type BensonChainSpec = sc_service::GenericChainSpec<GenesisConfig, Extensions>;
...
pub fn config_genesis(network_keys: NetworkKeys) -> GenesisConfig {
	...
	GenesisConfig {
		...
		babe: BabeConfig {
			authorities: vec![],
			epoch_config: Some(benson_runtime::BABE_GENESIS_EPOCH_CONFIG),
		},
...
pub(crate) mod tests {
	...
	pub fn integration_test_config_with_single_authority() -> BensonChainSpec {
	...
	pub fn integration_test_config_with_two_authorities() -> BensonChainSpec {
		BensonChainSpec::from_genesis(
			"Integration Test",
			"test",
			ChainType::Development,
