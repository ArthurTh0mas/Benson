...
use benson_executor::{self, NativeExecutor};
	...
	($config:expr) => {{
		...
		.with_rpc_extensions(
			|client, pool, _backend, fetcher, _remote_blockchain| -> Result<RpcExtension, _> {
				Ok(benson_rpc::create(
					client,
					pool,
					benson_rpc::LightDeps::none(fetcher),
				))
			},
		)?;
	...
	let service = ServiceBuilder::new_light::<Block, RuntimeApi, benson_executor::Executor>(config)?
		...
		.with_rpc_extensions(
			|client, pool, _backend, fetcher, remote_blockchain| -> Result<RpcExtension, _> {
				...
				Ok(benson_rpc::create(client, pool, Some(light_deps)))
			},
		)?
		.build()?;
...
mod tests {
	...
	use benson_runtime::{constants::asset::SPENDING_ASSET_ID, Address, Call, GenericAssetCall, UncheckedExtrinsic};
