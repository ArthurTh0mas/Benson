...
use benson_executor::NativeExecutor;
...
macro_rules! new_full_start {
	($config:expr) => {{
		...
		.with_rpc_extensions(|builder| -> Result<RpcExtension, _> {
			...
			Ok(benson_rpc::create_full(deps))
		})?;
...
type ConcreteClient = Client<
	...
	LocalCallExecutor<Backend<ConcreteBlock>, NativeExecutor<benson_executor::Executor>>,
	ConcreteBlock,
	benson_runtime::RuntimeApi,
>;
...
pub fn new_light(config: Configuration) -> Result<impl AbstractService, ServiceError> {
	...
	let service = ServiceBuilder::new_light::<Block, RuntimeApi, benson_executor::Executor>(config)?
		...
		.with_rpc_extensions(|builder| -> Result<RpcExtension, _> {
			...
			let light_deps = benson_rpc::LightDeps {
				remote_blockchain,
				fetcher,
				client: builder.client().clone(),
				pool: builder.pool(),
			};
			Ok(benson_rpc::create_light(light_deps))
		})?
		.build()?;
...
mod tests {
	...
	use benson_primitives::types::{AccountId, Block, DigestItem, Signature};
	use benson_runtime::constants::{currency::CENTS, time::SLOT_DURATION};
	use benson_runtime::{constants::asset::SPENDING_ASSET_ID, GenericAssetCall};
	use benson_runtime::{Call, UncheckedExtrinsic};
