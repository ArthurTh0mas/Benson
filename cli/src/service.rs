	...
	($config:expr) => {{
		...
		.with_import_queue(|_config, client, mut select_chain, _transaction_pool| {
			...
			let (grandpa_block_import, grandpa_link) =
				grandpa::block_import::<_, _, _, benson_runtime::RuntimeApi, _, _>(
					client.clone(),
					&*client,
					select_chain,
				)?;
		...
		.with_rpc_extensions(|client, pool| -> RpcExtension { benson_rpc::create(client, pool) })?;
	...
	($config:expr, $with_startup_data: expr) => {{
		...
		if is_authority {
			...
			let authority_discovery =
				authority_discovery::AuthorityDiscovery::new(service.client(), service.network(), dht_event_rx);
...
mod tests {
	...
	use benson_runtime::{BalancesCall, Call, UncheckedExtrinsic};
