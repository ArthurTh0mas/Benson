	...
	T: Into<std::ffi::OsString> + Clone,
{
	...
	match opt.subcommand {
		...
		Some(Subcommand::Inspect(cmd)) => {
			...
			>(&config)?;
			let inspect = node_inspect::Inspector::<benson_primitives::types::Block>::new(client);
		...
		// 	cmd.run::<benson_primitives::types::Block, benson_executor::Executor>(config)
		// }
			...
			match ChainSpec::from(config.expect_chain_spec().id()) {
				Some(ref c)
					if c == &ChainSpec::Development
						|| c == &ChainSpec::BensonNikau
						|| c == &ChainSpec::BensonAzalea => {}
