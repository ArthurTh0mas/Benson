	...
	E: IntoExit,
{
	...
	match parse_and_prepare::<CustomSubcommands, NoCustom, _>(&version, "benson-node", args) {
		ParseAndPrepare::Run(cmd) => cmd.run(
			...
			|exit, _cli_args, _custom_args, mut config: Config<_, _>| {
				...
				info!("  by Annie Lai Investments UNlimited, 2018-2020");
		...
		ParseAndPrepare::CustomCommand(CustomSubcommands::Factory(cli_args)) => {
			let mut config: Config<_, _> =
				sc_cli::create_config_with_db_path(load_spec, &cli_args.shared_params, &version, None)?;
